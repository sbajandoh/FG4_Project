...
def _visualize_boxes(count,image, boxes, classes, scores, category_index, **kwargs):
  return visualize_boxes_and_labels_on_image_array(
      count,image, boxes, classes, scores, category_index=category_index, **kwargs)


def _visualize_boxes_and_masks(count,image, boxes, classes, scores, masks,
...
def _visualize_boxes_and_keypoints(count,image, boxes, classes, scores, keypoints,
                                   category_index, **kwargs):
  return visualize_boxes_and_labels_on_image_array(
      count,
	  image,
...
def _visualize_boxes_and_masks_and_keypoints(
    count,image, boxes, classes, scores, masks, keypoints, category_index, **kwargs):
  return visualize_boxes_and_labels_on_image_array(
      count,
	  image,
      boxes,
...
def visualize_boxes_and_labels_on_image_array(
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    if scores is None or scores[i] > min_score_thresh:
      cv2.imwrite("detected/frame%d.jpg" % count, image)
      
      box = tuple(boxes[i].tolist())
      ymin, xmin, ymax, xmax = box
      f= open("frame%d.txt" % count,"w+")
      f.write(str(ymin))
      f.close()
